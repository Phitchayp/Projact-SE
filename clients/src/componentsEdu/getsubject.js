...
const CourseList = ({ courses, setListCheck }) => {
    ...
    const handleCheckboxChange = (id, isChecked, courseData) => {
        if (isChecked) {
            setListCheck(prevList => [...prevList, courseData]);
        } else {
            setListCheck(prevList => prevList.filter(item => item.courseid !== courseData.courseid));
        }
    };
        ...
        <div>
            {courses.map((course) => (
                <div key={course.courseid} className='CheckboxOpenCourse-Item'>
                    <input onChange={(e) => { handleCheckboxChange(course.courseid, e.target.checked, course) }} type='checkbox' id={`checkText${course.courseid}`} />
